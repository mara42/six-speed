      ...
      Async.forEachLimit(result.types, options.concurrency || 1, (testType, done) => {
          ...
          (function runAttempt() {
            ...
            options.runTest(testName, testType, result => {
              ...
              let {sample, ...tmp} = result.stats ?? {};
              fullStats[testType] = tmp;
        ...
        () => {
          ...
          log(`Full stats: ${JSON.stringify(fullStats, undefined, 2)}`);
  ...
  benchTest(test, type, callback, async) {
    try {
      SixSpeed.tests[test][type](fn => {
        ...
        bench.on('complete', () => {
          callback({result: bench.error ? `${bench.error}` : bench.hz, elapsed: bench.times.elapsed, count: bench.count, stats: bench.stats});
        });
        bench.run({async});
